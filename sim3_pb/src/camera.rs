...
impl Default for PanOrbitSettings {
    fn default() -> Self {
        PanOrbitSettings {
            pan_sensitivity: 0.0015,                // 1000 pixels per world unit
            orbit_sensitivity: 0.3f32.to_radians(), // 0.1 degree per pixel
            zoom_sensitivity: 0.01,
            scroll_action: Some(PanOrbitAction::Zoom),
            scroll_line_sensitivity: 24.0, // 1 "line" == 16 "pixels of motion"
...
pub fn spawn_camera(mut commands: Commands) {
    ...
    camera.state.center = Vec3::new(16.0, 16.0, 0.0);
    camera.state.radius = 30.0;
    camera.state.pitch = 45.0f32.to_radians();
    camera.state.yaw = 0.0f32.to_radians();

    // test location
    camera.state.center = Vec3::new(2.267082, 1.4004292, -1.4609228);
    camera.state.radius = 11.4867935;
    camera.state.pitch = 0.67020553;
    camera.state.yaw = -2.9059734;
...
pub fn pan_orbit_camera(
    ...
    for (settings, mut state, mut transform) in &mut q_camera {
        if kbd.just_pressed(MouseButton::Middle) {
            println!("{state:?}")
        }
        ...
        if kbd.pressed(MouseButton::Left) {
        ...
        if kbd.pressed(MouseButton::Right) {
        ...
        if any || state.is_added() {
            ...
            transform.rotation = Quat::from_euler(EulerRot::ZXY, state.yaw, state.pitch, 0.0);
